...
public class AdminDAO implements IAdminDAO{
	...
	public List<Object[]> checkCurrentMonthBill(String customerId) {
		Session session = HibernateUtil.getSessionFactory().openSession();
		String query = "select month(bill_generation_date) from bill where user_id = '" + customerId + "' order by bill_generation_date desc limit 1" ;
		NativeQuery nativeQuery = session.createSQLQuery(query); 
		List<Object[]> lastBillDate = nativeQuery.list();
		session.close();
		return lastBillDate;
	...
	public List<Object[]> calculateDue(String customerId) {
		Session session = HibernateUtil.getSessionFactory().openSession();
		String query = "select id, unit, price, bill_generation_date from bill where user_id = '" + customerId + "' and payment_status = 'not done'";
		NativeQuery nativeQuery = session.createSQLQuery(query); 
		List<Object[]> unpaidBillsList = nativeQuery.list();
		session.close();
